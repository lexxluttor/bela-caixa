...
function registrarCobrancas(clientes) {
  ...
  if(!lista.length){
    return Promise.resolve({ok:true,totalSalvos:0,historicoCobrancasProcessadas:0});
  }

  /*
   * IMPORTANTE:
   * a gravação oficial do histórico é feita diretamente pelo Apps Script.
   * Não passamos primeiro pelo sync geral, porque o sync pode estar ocupado
   * no exato momento em que a cobrança termina.
   */
  return chamarAppsScriptCobrancas_('registrarHistoricoCobrancas', {
    ...
    .then(function(resposta){
       ...
       * só apaga os históricos temporários depois da confirmação do servidor.
       */
      ...
      const processadas = Number(resposta && resposta.historicoCobrancasProcessadas || qtd);
      ...
      return Object.assign({}, resposta, {
        ok: true,
        totalSalvos: qtd,
        historicoCobrancasProcessadas: processadas
      });
    })
    .catch(function(erro){
      /*
       * Se o servidor não confirmou, o histórico não é apagado.
       * Mantemos uma fila pequena para o próximo sync tentar novamente.
       */
