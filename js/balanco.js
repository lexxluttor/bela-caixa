...
function balBuscarProduto(codigo){
  var alvo=balNormalizarCodigo(codigo);
  if(!alvo) return null;

  // Mantém exatamente a mesma busca utilizada no balanço original.
  // A otimização permanece somente na atualização visual da tabela.
  return balProdutosCategoria().find(function(p){
    return balCodigosProduto(p).indexOf(alvo)>=0;
  }) || null;
}
