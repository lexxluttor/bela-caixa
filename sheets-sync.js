/**
 * sheets-sync.js — v5 corrigido
 *
 * CORREÇÕES:
 * 1. URL /exec correta
 * 2. POST com URLSearchParams + no-cors
 * 3. Push antes do pull
 * 4. Pull não sobrescreve se planilha vazia
 * 5. scheduleSync exposto globalmente
 * 6. NOVO: controle de exclusões — itens deletados localmente
 *    não voltam no pull seguinte
 */
...
(function () {
  ...
  function saveDeleted(obj) {
    _rawSet("bm__deleted", JSON.stringify(obj));
  }
  function registerDeleted(sheet, ids) {
    const del = getDeleted();
    if (!del[sheet]) del[sheet] = [];
    ids.forEach(id => { if (!del[sheet].includes(String(id))) del[sheet].push(String(id)); });
    saveDeleted(del);
  }
  function filterDeleted(sheet, rows) {
    const del = getDeleted();
    const deletedIds = new Set((del[sheet] || []).map(String));
    return rows.filter(r => !deletedIds.has(String(r.id || "")));
  }

  /* ── detecta exclusões comparando snapshots ──────────────── */
  function detectAndRegisterDeletions(sheetKey, localKey, idField) {
    const prev = safeParse(localStorage.getItem("bm__prev_" + localKey) || "[]", []);
    const curr = readLocal(localKey);
    const prevIds = new Set(prev.map(r => String(r[idField] || r.id || "")));
    const currIds = new Set(curr.map(r => String(r[idField] || r.id || "")));
    const deletedIds = [...prevIds].filter(id => id && !currIds.has(id));
    if (deletedIds.length > 0) registerDeleted(sheetKey, deletedIds);
  ...
  function deserializePagamentos(rows, vendas, clientes) {
    ...
    return (rows||[]).map(r => { const vendaId=String(r.venda_id||""); let cid=""; if(vendaId.startsWith("cid:"))cid=vendaId.slice(4); if(!cid&&vendaMap.has(vendaId)){const venda=vendaMap.get(vendaId);const cli=clientMapByName.get(String(venda.cliNome||"").toLowerCase());if(cli)cid=cli.id;} return {id:String(r.id||""),vid:vendaId&&!vendaId.startsWith("cid:")?vendaId:"",cid:cid||"",val:Number(r.valor||0),forma:r.forma_pagamento||"",data:r.data||isoNow(),createdAt:r.data||isoNow(),updatedAt:r.data||isoNow()}; }).filter(x=>x.id||x.val);
  }
  ...
  function deserializeCreditos(rows, clientes) {
    const clientMapByName=new Map((clientes||[]).map(c=>[String(c.nome||"").toLowerCase(),c]));
    return (rows||[]).map(r => { const cli=clientMapByName.get(String(r.cliente||"").toLowerCase())||{}; return {id:String(r.id||""),cid:cli.id||"",cliNome:r.cliente||"",val:Number(r.valor||0),status:r.status||"aberto",vencimento:r.vencimento||"",data:r.vencimento||isoNow(),createdAt:r.vencimento||isoNow(),updatedAt:r.vencimento||isoNow()}; }).filter(x=>x.id||x.cliNome);
  }
  ...
  async function pushLocal() {
    // detecta o que foi deletado antes de enviar
    detectAndRegisterDeletions("clientes",     "clientes",   "id");
    detectAndRegisterDeletions("produtos",     "produtos",   "id");
    detectAndRegisterDeletions("vendas",       "vendas",     "id");
    detectAndRegisterDeletions("recebimentos", "pagamentos", "id");
    detectAndRegisterDeletions("cobrancas",    "creditos",   "id");
