...
var DB={
  ...
  set:function(k,v){
    var chave='bm_'+k,dados=JSON.stringify(v);
    try{localStorage.setItem(chave,dados);}catch(e){
      try{
        Object.keys(localStorage).forEach(function(key){
          if(/^bm_backup|^backup_[123]$|^bm_last_(sync|restore)_error$|^bm_backup_produtos_antes_(exclusao|balanco_)|^bm_backup_unificacao_produtos$/.test(key))localStorage.removeItem(key);
        });
      }catch(_){ }
      try{localStorage.setItem(chave,dados);}catch(e2){
